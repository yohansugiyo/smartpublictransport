...
public class SettingsActivity extends ActionBarActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		items.add(item);
		item = new HashMap<String, String>(2);
		item.put(FIRST, String.format(
				resources.getString(R.string._km_tracked),
				0.001 * StatisticCounter.getInstance().getTotalDistance()));
		item.put(SECOND, resources.getString(R.string.share_to_friend));
		items.add(item);

		SimpleAdapter adapter = new SimpleAdapter(this, items,
				android.R.layout.simple_list_item_2, new String[] { FIRST,
						SECOND }, new int[] { android.R.id.text1,
						android.R.id.text2 });
